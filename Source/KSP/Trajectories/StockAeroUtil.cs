        ...
        public static Vector3 SimAeroForce(Vessel _vessel, Vector3 v_wrld_vel, double altitude, double latitude=0)
        {
            ...
            for (int i = 0; i < _vessel.Parts.Count; ++i)
            {
                    ...
                    case Part.DragModel.CUBE:
                        ...
                        else
                        {
                            ...
                            catch (Exception)
                            {
                                ...
                                cubes.AddSurfaceDragDirection(-sim_dragVectorDirLocal, (float)mach, ref p_drag_data);
                                //Log.Info(String.Format("Trajectories: Caught NRE on Drag Initialization.  Should be fixed now.  {0}", e));
                            }
                ...
                if (!p.hasLiftModule)
                {
                    ...
                    //Log.Info("[Trajectories] bodyLift=" + bodyLift);
                    total_lift += bodyLift; 
                }
            ...
            // RETURN STUFF
            //Log.Info("[Trajectories] total_lift" + total_lift + " total_drag=" + total_drag);
